...
children: [
_buildSummaryCard('Total', '10 Leads', Icons.people, isDarkMode),
const SizedBox(width: 8),
_buildSummaryCard('Won', '0', Icons.check_circle, isDarkMode),
const SizedBox(width: 8),
_buildSummaryCard('Lost', '10', Icons.cancel, isDarkMode),
],
),
),
),
...
_buildFilterButton('Quotation Rejected', isDarkMode),
_buildFilterButton('Waiting for Demo', isDarkMode),
_buildFilterButton('Demo Scheduled', isDarkMode),
_buildFilterButton('Demo Cancelled', isDarkMode),
_buildFilterButton('Closed', isDarkMode),
],
),
const SizedBox(height: 10),
Expanded(
...
label,
style: GoogleFonts.poppins(
color: selectedLeadStatus == label
? (isDarkMode ? DarkTheme.textColor : AppTheme.textColor)
    : (isDarkMode ? DarkTheme.textColor.withOpacity(0.7) : AppTheme.secondaryTextColor),
),
),
...
final paginatedLeads = filteredLeads.asMap().entries.where((entry) => entry.key >= startIndex && entry.key < endIndex).toList();
